...
4. **Environment Configuration**: Copy the `.env.example` file to `.env` and configure your database and Redis settings.  
   ```bash
   cp .env.example .env
   ```
...
1. **Gacha Page**:  
   [http://localhost:8000/standard-banner](http://localhost:8000/standard-banner)  
   This is the main page for the gacha simulation.  
...
To update these images, go to **Admin Panel** at [http://localhost:8000/admin](http://localhost:8000/admin) and edit the respective data.  

### Gacha Result Images  
If a gacha result does not have an assigned image, the system will display a **default image** instead.  
To show the correct images for gacha results, update the data via the **Admin Panel** by uploading the appropriate images for weapons and characters.  
